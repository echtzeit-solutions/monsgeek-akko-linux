...
/// Set active profile
pub fn set_profile(profile: u8) -> CommandResult {
...
/// Set debounce time
pub fn set_debounce(ms: u8) -> CommandResult {
    match SyncKeyboard::open_any() {
...
/// Set polling rate
pub fn set_rate(rate: &str) -> CommandResult {
...
pub fn set_led(mode: &str, brightness: u8, speed: u8, r: u8, g: u8, b: u8) -> CommandResult {
    let mode_num = cmd::LedMode::parse(mode)
        .map(|m| m.as_u8())
        .unwrap_or_else(|| mode.parse().unwrap_or(1));

    match SyncKeyboard::open_any() {
        Ok(keyboard) => match keyboard.set_led(mode_num, brightness, speed, r, g, b, false) {
            Ok(_) => println!(
...
pub fn set_color_all(r: u8, g: u8, b: u8, layer: u8) -> CommandResult {
    match SyncKeyboard::open_any() {
        Ok(keyboard) => {
